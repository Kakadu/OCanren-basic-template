...
type 'a mystream = ('a, 'a mystream) Lib.HO.Stream.t [@@deriving gt ~options:{show}]

let prj_exn_mystream ra =
  let open Lib.HO.Stream in
...
let _ =
  ...
  run_exn 1 q qh
    (REPR (fun q -> ocanren { Lib.HO.append ((===)xs) ((===)ys) q } ))
...
module _ = struct
  ...
  let () =
    ...
    |> OCanren.Stream.iter (fun xs ->
           Format.printf "%s\n" (GT.show ground (GT.show GT.int) xs))
end
