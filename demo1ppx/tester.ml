...
let wrap onOK onFree i (name, x) = if x#is_open then onFree i name x else onOK i name x
let qh onOK onFree q () = List.iteri (wrap onOK onFree) @@ [ "q", q ]
let qrh onOK onFree q r () = List.iteri (wrap onOK onFree) @@ [ "q", q; "r", r ]

let qrsh onOK onFree q r s () =
  List.iteri (wrap onOK onFree) @@ [ "q", q; "r", r; "s", s ]
;;

let qrsth onOK onFree q r s t () =
...
let run_gen onOK onFree n num handler (repr, goal) =
  ...
  let () =
    try loop (run num goal handler) 1 with
    | NoMoreAnswers -> ()
  in
  printf "}\n%!"
;;
...
*)
let runR reifier printerNoFree printerR =
  run_gen
    (fun i name x ->
